...
public:
	...
	/** Utility for small UE4 backport tests that need to push a local root-motion delta through active modifiers. */
	UFUNCTION(BlueprintCallable, Category = "Motion Warping|Testing")
	FTransform ProcessRootMotionForTesting(const FTransform& InRootMotion, float DeltaSeconds);
...
protected:
	...
	/** Montage tracked by the UE4 backport's notify-window discovery. */
	UPROPERTY(Transient)
	TWeakObjectPtr<const UAnimMontage> ActiveMontage;
	...
	void AddModifiersFromMontage(const UAnimMontage* Montage, float CurrentPosition);
	...
	FTransform ProcessRootMotionPreConvertToWorld_UE4(const FTransform& InRootMotion, class UCharacterMovementComponent* CharacterMovementComponent);
};
